...
const Sound = (() => {
  ...
  /* A loot swoosh: white noise through a band-pass filter whose centre frequency
   * rises and falls (a sweep), with a matching volume swell. Optional stereo pan
   * spreads a burst of them across the room. */
  ...
  /* A green (within 25%): a single bright swoosh of loot hitting the bag. */
  function greenSector(t0) {
    whoosh(t0, { dur: 0.75, peak: 0.34, center: 860, pan: 0 });
  }

  /* A purple (within 10%, the jackpot): the chest groans open — several
   * staggered swooshes panned across the room, capped with a bright sparkle so
  ...
  /* Task accepted: the session-start sting — five rising "gear up" beeps, a
   * beat of silence, then a low surge as the grind begins (~1.8s). Original
   * composition; no game audio. */
  function lightsOut(t0) {
    const gap = 0.30;
    for (let i = 0; i < 5; i++) blip(t0 + i * gap, 700, 0.18, 0.22, { type: "square" });
    const go = t0 + 5 * gap + 0.22;     // "...task accepted!"
  ...
  /* Task complete: a fuller four-note ascending fanfare — our original
   * level-up-flavoured jingle (not the game's). */
