...
Speed:
  * seed_all is deterministic (committed entities + validation pipeline) but
    costs seconds, and the per-test fixtures build one fresh database per
    test — that setup dominated the whole suite. Seeding a FRESH path now
    copies a template file seeded once per session instead. Reseeding an
    EXISTING database still runs the real pipeline, because its contract
    (drop only the data tables, preserve accounts/flags) is itself under test.
...
def _templated_seed_all(db_path=None):
    ...
    if _template_db is None:
        _template_db = Path(tempfile.mkdtemp(prefix="scapemaster-seed-template-")) / "template.db"
