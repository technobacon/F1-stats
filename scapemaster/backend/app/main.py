...
# way). Inline style attributes/<style> blocks are used throughout, hence
# 'unsafe-inline' for styles only; fonts are self-hosted.
_CSP = (
    "default-src 'self'; "
    "script-src 'self'; "
...
# Training Grounds throttle. Practice serves unlimited questions from the SAME
# bank as the Daily, and verify reveals the true answer after any guess — so
# with no server-side limit a script could farm the whole answer key overnight
# and then ace every Daily (a client-side penalty only slows humans down; it is
# trivially bypassed by calling the API directly). The window is generous for a
# human (a question takes 10s+ to read and answer) but makes scripted farming
# of the bank take days instead of minutes. In-memory sliding window keyed by
# client IP, same pattern as the login limiter (auth.py); Redis in production.
...
def _check_practice_allowed(client_ip: str) -> None:
    ...
    if len(hits) >= _PRACTICE_MAX_PER_WINDOW:
        ...
        raise HTTPException(
            429,
            "Slow down, adventurer — you've hit the Training Grounds limit. "
...
def god_leaderboard(conn: Conn, period: str = "all"):
...
def leaderboard_god(conn: Conn, period: str = "all", user: dict = Depends(require_user)):
...
def set_god(conn: Conn, req: SetGodRequest, user: dict = Depends(require_user)):
