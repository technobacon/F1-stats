...
def practice_question(conn: Conn, request: Request,
    ...
    An optional focus narrows the draw to one category and/or era decade window
    (see service.PRACTICE_ERAS); an empty focus falls back to the full bank with
    focus_matched=False. Rate-limited per client so the bank can't be
    scripted-farmed for answers."""
...
def quiz_hint(conn: Conn, req: HintRequest):
    """Radio the Pit Wall about a served question: returns a salted, non-revealing
    band that contains the answer and marks the token so verify() applies the
    call's score cost. Idempotent per token — asking twice returns the same band
...
def quiz_daily_field(conn: Conn, anon_id: str | None = None,
    ...
    """Where the caller finished among everyone who has played today's Daily so
    far — members and guests alike (identity comes from the bearer token when
    present, else the guest anon_id). Powers the post-session 'P4 of 23 in
