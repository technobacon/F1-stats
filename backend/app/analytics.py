...
Two hard rules:
  ...
  * The reporting endpoint is gated by ``F1_ANALYTICS_TOKEN`` (see main.py), so
...
EVENT_NAMES = frozenset({
    ...
    "team_select",     # chose a constructor faction (props.team)
})
...
def token_configured() -> bool:
    return bool(os.environ.get("F1_ANALYTICS_TOKEN", "").strip())


def token_matches(token: str | None) -> bool:
    """Constant-time check of a bearer token against F1_ANALYTICS_TOKEN."""
    import hmac
    expected = os.environ.get("F1_ANALYTICS_TOKEN", "").strip()
